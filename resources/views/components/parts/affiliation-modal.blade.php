...
　<div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-14  ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
    ...
    {{ Form::model($lifters, ['method'=>'post', 'route'=>'admins.lifters.create']) }}
    {{ Form::token() }}
        ...
        <div class="p-4 overflow-y-auto">
            <ul>
                <li class="flex mb-4">
                    {{ Form::label('affiliation', '所属', ['class' => 'shrink-0 w-24']) }}
                    <p class="shrink-0 w-20 text-[#FF0404]">【必須】</p>
                    {{ Form::text('affiliation', null, ['placeholder'=>'所属', 'class' => 'w-full placeholder:text-slate-400 border-slate-300 rounded-md' ]) }}
                </li>
            </ul>
        </div>
        <div class="flex justify-center items-center gap-x-2 py-3 px-4 border-t dark:border-gray-700">
            {{ Form::submit('登録する', ['class'=>'py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-black text-white hover:bg-opacity-80 focus:outline-none focus:ring-2 focus:ring-spacity-80 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800']) }}
        </div>
    {{ Form::close() }}
    </div>
  </div>
